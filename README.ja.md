...
どちらのエントリーポイントも同じ `run:` コマンドを同じ環境で実行します。違いはトリガーだけです。完全な action プラグインの例は [examples/plugins/diff-review](examples/plugins/diff-review) を参照してください。
...
- **ポップアップは `JIN_*` 環境変数を継承しません。** `jin pane popup` / `jin pane split` は tmux が新規に spawn したプロセスでコマンドを実行するため、ポップアップに必要なデータは環境変数ではなくコマンドライン引数として渡してください。このパターンは [examples/plugins/diff-review](examples/plugins/diff-review) を参照してください。
